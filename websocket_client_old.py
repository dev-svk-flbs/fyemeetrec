...
class MeetingRecorderClient:
    ...
    async def monitor_recording(self):
        ...
        while self.current_recording:
            ...
            print(f"\n  Elapsed: {int(elapsed)}s / {self.recording_duration}s (Remaining: {int(remaining)}s)")
            ...
            if not recording_active and elapsed < self.recording_duration:
                print("  Recording stopped prematurely on Flask side!")
            ...
            if not ffmpeg_ok and recording_active:
                print("  FFmpeg not detected but Flask reports recording active")
